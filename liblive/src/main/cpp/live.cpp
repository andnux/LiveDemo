...
#define LOGI(FORMAT, ...) __android_log_print(ANDROID_LOG_INFO,"andnux",FORMAT,##__VA_ARGS__)
...
void throwNativeError(JNIEnv *env, int code) {
    if (jobj_push_native != NULL && jmid_throw_native_error != NULL) {
...
Java_top_andnux_liblive_jni_PushNative_startPush(JNIEnv *env, jobject instance, jstring url_jstr) {
    jobj_push_native = env->NewGlobalRef(instance);

    jclass jcls_push_native_tmp = env->GetObjectClass(instance);
    jcls_push_native = static_cast<jclass>(env->NewGlobalRef(jcls_push_native_tmp));
    ...
    rtmp_path = static_cast<char *>(malloc(strlen(url_cstr) + 1));
...
extern "C"
JNIEXPORT void JNICALL
Java_top_andnux_liblive_jni_PushNative_stopPush(JNIEnv *env, jobject instance) {
    is_pushing = FALSE;
}

extern "C"
JNIEXPORT void JNICALL
Java_top_andnux_liblive_jni_PushNative_release(JNIEnv *env, jobject instance) {

    if (jcls_push_native != NULL) {
        env->DeleteGlobalRef(jcls_push_native);
    }
    if (jobj_push_native != NULL) {
        env->DeleteGlobalRef(jobj_push_native);
    }
//    if (jmid_throw_native_error != NULL) {
//        env->DeleteGlobalRef(reinterpret_cast<jobject>(jmid_throw_native_error));
//    }
}
...
extern "C"
JNIEXPORT void JNICALL
Java_top_andnux_liblive_jni_PushNative_setVideoOptions(JNIEnv *env, jobject instance, jint width,
                                                       jint height, jint bitrate, jint fps) {
    LOGD("%s", "开始设置视频参数。。。");
    x264_param_t param;
    int ret = -1;
    // 1.x264_param_default_preset设置
...
extern "C"
JNIEXPORT void JNICALL
Java_top_andnux_liblive_jni_PushNative_setAudioOptions(JNIEnv *env, jobject instance,
...
extern "C"
JNIEXPORT void JNICALL
Java_top_andnux_liblive_jni_PushNative_sendVideo(JNIEnv *env, jobject instance, jbyteArray buffer) {
//NV21->YUV420P
...
extern "C"
JNIEXPORT void JNICALL
Java_top_andnux_liblive_jni_PushNative_sendAudio(JNIEnv *env, jobject instance, jbyteArray data_,
                                                 jint length) {
    int *pcmbuf;
    unsigned char *bitbuf;
    jbyte *b_buffer = env->GetByteArrayElements(data_, 0);
    pcmbuf = reinterpret_cast<int *>((short *) malloc(nInputSamples * sizeof(int)));
    ...
    while (nByteCount < nBufferSize) {
        int audioLength = static_cast<int>(nInputSamples);
